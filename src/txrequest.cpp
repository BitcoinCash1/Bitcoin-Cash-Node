...
inline constexpr TxId kTxIdZero{}; //! Used in below code for the "lowest" possible txid (all 0's)
 ...
 * Also note that the sorting order of ByTxIdView relies on the specific order of values in this enum.
 *
...
enum class State : uint8_t {
    ...
    CANDIDATE_READY,
    /** The best CANDIDATE for a given txid; only if there is no REQUESTED announcement already for that txid.
...
/** An announcement. This is the data we track for each txid that is announced to us by each peer. */
struct Announcement {
    ...
    /** Whether this announcement is selected. There can be at most 1 selected peer per txid. */
    ...
    Announcement(const TxId& txid, NodeId peer, bool preferred, std::chrono::microseconds reqtime,
                 SequenceNumber sequence)
          : m_txid(txid), m_time(reqtime), m_peer(peer), m_sequence(sequence), m_preferred(preferred),
...
public:
    ...
    Priority operator()(const TxId& txid, NodeId peer, bool preferred) const {
        uint64_t low_bits = CSipHasher(m_k0, m_k1).Write(txid.begin(), txid.size()).Write(peer).Finalize() >> 1;
...
//
// Uses:
// * Looking up existing announcements by peer/txid, by checking both (peer, false, txid) and
//   (peer, true, txid).
...
struct ByPeerViewExtractor {
    ...
    result_type operator()(const Announcement& ann) const {
        return ByPeerView{ann.m_peer, ann.m_state == State::CANDIDATE_BEST, ann.m_txid};
    }
};
...
//   announcement exists for that txid.
// * Determining when no more non-COMPLETED announcements for a given txid exist, so the COMPLETED ones can be
//   deleted.
struct ByTxId {};
...
public:
    ByTxIdViewExtractor(const PriorityComputer& computer) : m_computer(computer) {}
    using result_type = ByTxIdView;
    result_type operator()(const Announcement& ann) const {
        const Priority prio = (ann.m_state == State::CANDIDATE_READY) ? m_computer(ann) : 0;
        return ByTxIdView{ann.m_txid, ann.m_state, prio};
    }
};
...
using Index = boost::multi_index_container<
    ...
    boost::multi_index::indexed_by<
        ...
        boost::multi_index::ordered_non_unique<boost::multi_index::tag<ByTxId>, ByTxIdViewExtractor>,
...
template<typename Tag>
using ConstIter = typename Index::index<Tag>::type::const_iterator;
...
struct TxIdInfo {
    ...
    //! Number of REQUESTED announcements for this txid (at most one; mutually exclusive with CANDIDATE_BEST).
...
std::map<TxId, TxIdInfo> ComputeTxIdInfo(const Index& index, const PriorityComputer& computer) {
    ...
    for (const Announcement& ann : index) {
        ...
        // Also keep track of which peers this txid has an announcement for (so we can detect duplicates).
...
public:
    void SanityCheck() const {
        ...
        // Calculate per-txid statistics from m_index, and validate invariants.
        for (auto& item : ComputeTxIdInfo(m_index, m_computer)) {
            TxIdInfo& info = item.second;

            // Cannot have only COMPLETED peer (txid should have been forgotten already)
...
private:
    ...
    void Modify(Iter<Tag> it, Modifier modifier) {
        auto peerit = m_peerinfo.find(it->m_peer);
        const bool found = peerit != m_peerinfo.end(); // should always be true, but we check to prevent UB
        if (found) {
            peerit->second.m_completed -= it->m_state == State::COMPLETED;
            peerit->second.m_requested -= it->m_state == State::REQUESTED;
        }
    ...
    void PromoteCandidateReady(Iter<ByTxId> it) {
        ...
        Modify<ByTxId>(it, [](Announcement& ann){ ann.m_state = State::CANDIDATE_READY; });
        // The following code relies on the fact that the ByTxId is sorted by txid, and then by state (first
        // _DELAYED, then _READY, then _BEST/REQUESTED). Within the _READY announcements, the best one (highest
        // priority) comes last. Thus, if an existing _BEST exists for the same txid that this announcement may
        ...
        if (it_next == m_index.get<ByTxId>().end() || it_next->m_txid != it->m_txid ||
            ...
            // This is the new best CANDIDATE_READY, and there is no IsSelected() announcement for this txid
            // already.
            Modify<ByTxId>(it, [](Announcement& ann){ ann.m_state = State::CANDIDATE_BEST; });
            ...
            if (priority_new > priority_old) {
                ...
                Modify<ByTxId>(it_next, [](Announcement& ann){ ann.m_state = State::CANDIDATE_READY; });
                Modify<ByTxId>(it, [](Announcement& ann){ ann.m_state = State::CANDIDATE_BEST; });
            }
        }
    }
    ...
    void ChangeAndReselect(Iter<ByTxId> it, State new_state) {
        ...
        if (it->IsSelected() && it != m_index.get<ByTxId>().begin()) {
            ...
            // announcement in the ByTxId index.
            if (it_prev->m_txid == it->m_txid && it_prev->m_state == State::CANDIDATE_READY) {
                // If one such CANDIDATE_READY exists (for this txid), convert it to CANDIDATE_BEST.
                Modify<ByTxId>(it_prev, [](Announcement& ann){ ann.m_state = State::CANDIDATE_BEST; });
            }
        }
        Modify<ByTxId>(it, [new_state](Announcement& ann){ ann.m_state = new_state; });
    }

    //! Check if 'it' is the only announcement for a given txid that isn't COMPLETED.
    bool IsOnlyNonCompleted(ConstIter<ByTxId> it) const {
        assert(it != m_index.get<ByTxId>().end());
        assert(it->m_state != State::COMPLETED); // Not allowed to call this on COMPLETED announcements.

        // This announcement has a predecessor that belongs to the same txid. Due to ordering, and the
        ...
        // This announcement has a successor that belongs to the same txid, and is not COMPLETED.
        if (const auto next_it = std::next(it); next_it != m_index.get<ByTxId>().end()
                                                && next_it->m_txid == it->m_txid
                                                && next_it->m_state != State::COMPLETED) {
            return false;
        }
     ...
     *  txid, they are deleted. If this was a REQUESTED announcement, and there are other CANDIDATEs left, the
    ...
    bool MakeCompleted(Iter<ByTxId> it) {
        ...
        if (IsOnlyNonCompleted(it)) {
            // This is the last non-COMPLETED announcement for this txid. Delete all.
            const TxId txid = it->m_txid;
            do {
                it = Erase<ByTxId>(it);
...
public:
    Impl(bool deterministic)
        : m_computer(deterministic),
          // Explicitly initialize m_index as we need to pass a reference to m_computer to ByTxIdViewExtractor.
          m_index(boost::make_tuple(
              boost::make_tuple(ByPeerViewExtractor(), std::less<ByPeerView>()),
              boost::make_tuple(ByTxIdViewExtractor(m_computer), std::less<ByTxIdView>()),
              boost::make_tuple(ByTimeViewExtractor(), std::less<ByTimeView>())
              )) {}

    // Disable copying and assigning (a default copy won't work due the stateful ByTxIdViewExtractor).
    Impl(const Impl&) = delete;
    Impl& operator=(const Impl&) = delete;

    void DisconnectedPeer(NodeId peer) {
        auto& index = m_index.get<ByPeer>();
        auto it = index.lower_bound(ByPeerView{peer, false, kTxIdZero});
        while (it != index.end() && it->m_peer == peer) {
            ...
            // - 'it' is not the only non-COMPLETED announcement for its txid. This means it will be deleted, but
            ...
            // - 'it' is the only non-COMPLETED announcement for its txid. This means it will be deleted along
            //   with all other announcements for the same txid - which may include std::next(it). However, other
            //   than 'it', no announcements for the same peer can be affected (due to (peer, txid) uniqueness).
            //   In other words, the situation where std::next(it) is deleted can only occur if std::next(it)
            //   belongs to a different peer but the same txid as 'it'. This is covered by the first bulletpoint
            //   already, and we'll have set it_next to end().
            auto it_next = std::next(it);
            if (it_next != index.end() && it_next->m_peer != peer) it_next = index.end(); // force end on peer change
            // If the announcement isn't already COMPLETED, first make it COMPLETED (which will mark other
            // CANDIDATEs as CANDIDATE_BEST, or delete all of a txid's announcements if no non-COMPLETED ones are
            // left).
    ...
    void ForgetTxId(const TxId& txid) {
        auto it = m_index.get<ByTxId>().lower_bound(ByTxIdView{txid, State::CANDIDATE_DELAYED, 0});
        while (it != m_index.get<ByTxId>().end() && it->m_txid == txid) {
            it = Erase<ByTxId>(it);
        }
    }

    void ReceivedInv(NodeId peer, const TxId& txid, bool preferred, std::chrono::microseconds reqtime) {
        // Bail out if we already have a CANDIDATE_BEST announcement for this (txid, peer) combination. The case
        ...
        if (m_index.get<ByPeer>().count(ByPeerView{peer, true, txid})) return;

        // Try creating the announcement with CANDIDATE_DELAYED state (which will fail due to the uniqueness
        // of the ByPeer index if a non-CANDIDATE_BEST announcement already exists with the same txid and peer).
        // Bail out in that case.
        auto ret = m_index.get<ByPeer>().emplace(txid, peer, preferred, reqtime, m_current_sequence);
    ...
    std::vector<TxId> GetRequestable(NodeId peer, std::chrono::microseconds now) {
        ...
        auto it_peer = m_index.get<ByPeer>().lower_bound(ByPeerView{peer, true, kTxIdZero});
        while (it_peer != m_index.get<ByPeer>().end() && it_peer->m_peer == peer
               && it_peer->m_state == State::CANDIDATE_BEST) {
            selected.push_back(&*it_peer);
            ++it_peer;
        }
        ...
        for (const Announcement *ann : selected) {
            ret.push_back(ann->m_txid);
        }
        return ret;
    }

    void RequestedTx(NodeId peer, const TxId& txid, std::chrono::microseconds expiry) {
        ...
        if (it == m_index.get<ByPeer>().end()) {
            ...
            // other than ForgetTxId and GetRequestable in between, this branch will never execute (as txids
            ...
            if (it == m_index.get<ByPeer>().end() || (it->m_state != State::CANDIDATE_DELAYED
                ...
                // txid wasn't tracked at all (and the caller should have called ReceivedInv), or it was already
                // requested and/or completed for other reasons and this is just a superfluous RequestedTx call.
                return;
            }

            // Look for an existing CANDIDATE_BEST or REQUESTED with the same txid. We only need to do this if the
            ...
            auto it_old = m_index.get<ByTxId>().lower_bound(ByTxIdView{txid, State::CANDIDATE_BEST, 0});
            if (it_old != m_index.get<ByTxId>().end() && it_old->m_txid == txid) {
                if (it_old->m_state == State::CANDIDATE_BEST) {
                    ...
                    // REQUESTED announcement per txid (but not both simultaneously), so we have to convert any
                    ...
                    Modify<ByTxId>(it_old, [](Announcement& ann) { ann.m_state = State::CANDIDATE_READY; });
                    ...
                    Modify<ByTxId>(it_old, [](Announcement& ann) { ann.m_state = State::COMPLETED; });
                }
            }
        }
...
void TxRequestTracker::ForgetTxId(const TxId& txid) { m_impl->ForgetTxId(txid); }
...
void TxRequestTracker::ReceivedInv(NodeId peer, const TxId& txid, bool preferred,
                                   std::chrono::microseconds reqtime) {
    m_impl->ReceivedInv(peer, txid, preferred, reqtime);
}

void TxRequestTracker::RequestedTx(NodeId peer, const TxId& txid, std::chrono::microseconds expiry) {
