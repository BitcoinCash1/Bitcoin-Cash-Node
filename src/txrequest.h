 ...
 *
 * - No transaction is requested as long as another request for the same txid is outstanding (it needs to fail
 ...
 *     ForgetTxId, which removes all announcements across all peers with the specified txid.
 *   - If for a given txid only already-failed announcements remain, they are all forgotten.
 *
...
public:
    ...
    // Conceptually, the data structure consists of a collection of "announcements", one for each peer/txid
    // combination:
    //
    ...
    //   prevent requesting them again. If only COMPLETED announcements for a given txid remain (so no CANDIDATE
     ...
     *
     * Does nothing if one already exists for that (txid, peer) combination (whether it's CANDIDATE, REQUESTED, or
     * COMPLETED).
     */
     ...
     * for the same txid will not trigger new ReceivedInv calls, at least in the short term after this call.
     */
     ...
     *    announcements with the same txid (subject to preferredness rules, and tiebreaking using a deterministic
     *    salted hash of peer and txid).
     *  - The selected announcements are returned in announcement order (even if multiple were added at the same time,
     *    or when the clock went backwards while they were being added). This is done to minimize disruption from
     *    dependent transactions being requested out of order: if multiple dependent transactions are announced
     ...
     *
     * If no CANDIDATE announcement for the provided peer and txid exists, this call has no effect. Otherwise:
     *  - That announcement is converted to REQUESTED.
     *  - If any other REQUESTED announcement for the same txid already existed, it means an unexpected request
     ...
     *  provided peer and txid, nothing happens.
     *
     * It should be called whenever a transaction or NOTFOUND was received from a peer. When the transaction is
     * not needed entirely anymore, ForgetTxId should be called instead of, or in addition to, this call.
     */
