...
struct Initializer
{
    Initializer()
    {
        for (uint8_t txhash = 0; txhash < MAX_TXIDS; txhash += 1) {
            CSHA256().Write(&txhash, 1).Finalize(TXIDS[txhash].data());
        }
        int i = 0;
    ...
    void GetRequestable(int peer)
    {
        ...
        for (int txhash = 0; txhash < MAX_TXIDS; ++txhash) {
            ...
            if (ann.m_state == State::CANDIDATE && GetSelected(txhash) == peer) {
                result.emplace_back(ann.m_sequence, txhash);
            }
        }
    ...
    while (it != buffer.end()) {
        ...
        switch (cmd) {
            ...
            case 4: // No longer need tx
                ...
                tester.ForgetTxHash(txidnum % MAX_TXIDS);
                break;
            case 5: // Received immediate preferred inv
            case 6: // Same, but non-preferred.
                peer = it == buffer.end() ? 0 : *(it++) % MAX_PEERS;
                txidnum = it == buffer.end() ? 0 : *(it++);
                tester.ReceivedInv(peer, txidnum % MAX_TXIDS,cmd & 1, std::chrono::microseconds::min());
                break;
            ...
            case 8: // Same, but non-preferred.
                ...
                txidnum = it == buffer.end() ? 0 : *(it++);
                delaynum = it == buffer.end() ? 0 : *(it++);
                tester.ReceivedInv(peer, txidnum % MAX_TXIDS, cmd & 1, tester.Now() + DELAYS[delaynum]);
                break;
            case 9: // Requested tx from peer
                peer = it == buffer.end() ? 0 : *(it++) % MAX_PEERS;
                txidnum = it == buffer.end() ? 0 : *(it++);
                delaynum = it == buffer.end() ? 0 : *(it++);
                tester.RequestedTx(peer, txidnum % MAX_TXIDS, tester.Now() + DELAYS[delaynum]);
                break;
