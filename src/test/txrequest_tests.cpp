 ...
 * against a TxRequestTracker in parallel. Every test has its own unique txids and NodeIds which are not
 * reused in other tests, and thus they should be independent from each other. Running them in parallel however
 * means that we verify the behavior (w.r.t. one test's txids and NodeIds) even when the state of the data
...
public:
    ...
    void ReceivedInv(NodeId peer, const TxId& txid, bool pref, std::chrono::microseconds reqtime)
    {
        auto& runner = m_runner;
        runner.actions.emplace_back(m_now, [=,&runner]() {
    ...
    void RequestedTx(NodeId peer, const TxId& txid, std::chrono::microseconds exptime)
    {
        auto& runner = m_runner;
        runner.actions.emplace_back(m_now, [=,&runner]() {
     ...
     */
    void Check(NodeId peer, const std::vector<TxId>& expected, size_t candidates, size_t inflight,
    ...
    /** Generate a random txid, whose priorities for certain peers are constrained.
     *
     * For example, NewTxId({{p1,p2,p3},{p2,p4,p5}}) will generate a txid T such that both:
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    scenario.ReceivedInv(peer, txid, preferred, immediate ? MIN_TIME : scenario.Now() + delay);
    if (immediate) {
...
void BuildPriorityTest(Scenario& scenario, int config)
{
    ...
    bool prio1 = config & 1;
    auto txid = prio1 ? scenario.NewTxId({{peer1, peer2}}) : scenario.NewTxId({{peer2, peer1}});
...
void BuildBigPriorityTest(Scenario& scenario, int peers)
{
    ...
    // Find a txid whose txid prioritization is consistent with the required ordering within pref_peers and
    // within npref_peers.
 ...
 *
 *  Note: This was backported from core and modified to not distinguis between wtxid and txid,
 *        and adapted to our codebase as such. The test still passes so we have left it in
 *        for completeness; please disregard comments related to wtxid. -Calin
 */
void BuildWtxidTest(Scenario& scenario, int config)
{
    ...
    if (config & 1) {
        ...
        if (InsecureRandBool()) scenario.AdvanceTime(RandomTime8s());
        scenario.ReceivedInv(peerW, txid, !(config & 2), reqtimeW);
    } else {
        scenario.ReceivedInv(peerW, txid, !(config & 2), reqtimeW);
        if (InsecureRandBool()) scenario.AdvanceTime(RandomTime8s());
        scenario.ReceivedInv(peerT, txid, config & 2, reqtimeT);
    ...
    // Let time pass if needed, and check that the preferred announcement (txid)
    // is correctly to-be-requested.
    ...
    if (config & 2) {
        ...
        scenario.Check(peerW, {txid}, 1, 0, 0, "w4");
    }
    ...
    if (config & 2) {
        ...
        scenario.RequestedTx(peerW, txid, scenario.Now() + expiry);
    ...
    if (config & 2) {
        ...
        scenario.Check(peerW, {txid}, 1, 0, 0, "w10");
    } else {
...
void BuildTimeBackwardsTest(Scenario& scenario)
{
    ...
    scenario.Check(peer2, {txid}, 1, 0, 0, "r10"); // Request goes back to peer2.
...
/** Add to scenario a test that involves RequestedTx() calls for txids not returned by GetRequestable. */
void BuildWeirdRequestsTest(Scenario& scenario)
{
    ...
    // Also announce txid1 from peer2 now, so that the txid isn't forgotten when the peer1 request expires.
    ...
    // If peer2 now (normally) requests txid2, the existing request by peer1 becomes COMPLETED.
    if (InsecureRandBool()) scenario.AdvanceTime(RandomTime8s());
